...
for event in cal.walk('vevent'):
    ...
    if dtstart > time_min and dtstart < time_max:
        ...
        if "rrule" in event:
            rule = rrule.rrulestr(event.get('rrule').to_ical().decode("utf-8") , dtstart=event.get('dtstart').dt)
            rule = rrule.rruleset()
            ...
            updated_events = [ e for e in cal.walk('vevent') if e.get("uid") == event.get("uid") and "RECURRENCE-ID" in e]
            updated_starttimes = [ e.get("RECURRENCE-ID").dt for e in updated_events ]

            for dtstart_rec in rule.between(time_min, time_max):
                #send_mail(event, dtstart_rec)
                if dtstart_rec in updated_starttimes:
                    continue
